...
public class FoodBuilder {
    ...
    private static double calculateHumidity(Knowledge knowledge) {
        double value = ((1 -(double) knowledge.getValue(Corral.ALIMENTO_MATERIA_SECA.name())) * calculateMateriaSeca(knowledge)/(double) knowledge.getValue(Corral.ALIMENTO_MATERIA_SECA.name()));

        return round(value);
    }

    private static double calculatePremezclaMineral(Knowledge knowledge) {
        double value = ((double) knowledge.getValue(Corral.ALIMENTO_PREMEZCLA_MINERAL.name())) * calculateMateriaSeca(knowledge);

        return round(value);
    }

    private static double calculateExpellerSoja(Knowledge knowledge) {
        double value = ((double) knowledge.getValue(Corral.ALIMENTO_EXPELLER_SOJA.name())) * calculateMateriaSeca(knowledge);

        return round(value);
    }

    private static double calculateMaiz(Knowledge knowledge) {
        double value = ((double) knowledge.getValue(Corral.ALIMENTO_MAIZ.name())) * calculateMateriaSeca(knowledge);

        return round(value);
    }

    private static double calculateSiloSorgo(Knowledge knowledge) {
        double value = ((double) knowledge.getValue(Corral.ALIMENTO_SILO_SORGO.name())) * calculateMateriaSeca(knowledge);
    ...
    private static double calculateMateriaSeca(Knowledge knowledge) {
        ...
        double value = ((double) knowledge.getValue(Corral.PESO_ACTUAL.name())) *
                ((double) knowledge.getValue(Corral.PORCENTAJE_PESO_MATERIA_SECA.name())) *
                ((double) ((Integer)knowledge.getValue(Corral.CABEZAS.name()))) *
